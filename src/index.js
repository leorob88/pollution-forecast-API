//
// This API is based on a 3-ways search by the user:
//
// - specific location (name)
// - locations with a common name (keyword)
// - user location (position)
//
// As such, it seeks to handle any possible situation generated by the user, based on main concepts and inputs, so:
//
// - it expects a viable input (i.e. a name or user position) to search for; if no input is found, the user is warned and can retry
// - if the input was viable but the fetch could not find proper results, the user is warned and can try a different search
//
// Details about the results:
//
// - the name search gets the result which the fetch "considers" the most appropriate to the user input, sticking to the specific name
// - the keyword search is likely to find locations with a name which is different than the one provided;
//   '-- if one result is found, it gets automatically read and that's that
//   '-- if two or more results are found, a list appears and the user can choose the result they consider the most appropriate
// - geolocation is not always precise, so the user can be located somewhere globally near their true position
// - the formula to calculate the distance between the user and the result is slightly inaccurate due to the shape of Earth
//
// Finally, I chose to give this API a basic layout in order to make it simple and easily handle its appearance on different devices.
//
