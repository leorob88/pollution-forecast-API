    ...
    if (data.data == "Unknown station" || (searching == 2 && data.data.length == 0)){
      ...
      if (searching == 1){
        ...
        //if not, tell user name result was not found
          ...
          document.getElementById("answer").textContent = "I couldn't find any stations for pollution detection in the location you searched for.";
        }
      }
      //if search was by keyword,
        ...
        //if not, tell user keyword result was not found
          ...
          document.getElementById("answer").textContent = "I couldn't find any stations for pollution detection. Be sure to provide a proper location keyword.";
        }
      }
      //if search was by geolocation,
        ...
        //if not, tell user geolocation result was not found
          ...
          document.getElementById("answer").textContent = "I couldn't find any stations for pollution detection. Be sure to provide your current position";
        }
      }
    }
    //if response is found
      ...
      //if search WAS NOT by keyword (unique result)
        ...
        if (searching == 1) {
          ...
          document.getElementById("answer").textContent = `The estimated AQI for ${data.data.city.name} has a value of ${aqi}. The pollution rate is ${quality(aqi)}.`;
        }
        //if search was by geolocation
        else if (searching == 3) {
          //tell user the result and quality for their current position (nearest)
          document.getElementById("answer").textContent = `The nearest station to your estimated position is in ${data.data.city.name}. The estimated AQI has a value of ${aqi}. The pollution rate is ${quality(aqi)}.`;
        }
        ...
        if (far != null || far != undefined) {
          document.getElementById("answer").textContent += `The estimated distance from your position is about ${far} kilometers.`;
        }
      }
    }
    //optional further info
    document.getElementById("answer").textContent += ` For further details, you can check out the reference website infos <a target="_blank" href="https://www.airnow.gov/aqi/aqi-basics/">here</a>.`;
  });
}
...
function selection(){
  ...
  if (index > -1){
    ...
    document.getElementById("answer").textContent += `The estimated AQI for ${currentResult.station.name} has a value of ${aqi}. The pollution rate is ${quality(aqi)}.`;
    //if user position (and distance) is known, tell also the user how far they are from the stated station
    if (far != null || far != undefined) {
      document.getElementById("answer").textContent += `The estimated distance from your position is about ${far} kilometers.`;
    }
